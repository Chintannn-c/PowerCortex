...
class EmailService:
    ...
    def send_reset_token(to_email: str, token: str) -> bool:
        subject = "PowerCortex Password Reset Verification"
        html_content = f"""
        <html>
            <body style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
                ...
                <p>To reset your password, please enter the following 6-digit verification code in the application:</p>
                <div style="background-color: #F3F4F6; padding: 15px; border-radius: 8px; font-size: 24px; font-weight: bold; letter-spacing: 4px; text-align: center; margin: 20px 0; color: #1E3A8A; border: 1px solid #E5E7EB;">
                    {token}
                </div>
